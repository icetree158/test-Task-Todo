import React, { FC } from 'react'
import { useAppDispatch } from '../../hooks/redux';
import { ITodo } from '../../model/ITodo';
import { removeTodoByIndex } from '../../store/todosSlice';
import "./TodoCard.scss"
interface PropsTodoCard {
  setModal: (modal: boolean) => void;
  setactiveCard: (info: ITodo) => void;

  name: string;
  description: string;


}

const TodoCard: FC<PropsTodoCard> = ({ name, description, setModal, setactiveCard }) => {
  const dispath = useAppDispatch()
  const editTodo = () => {
    setModal(true)
    setactiveCard({
      name: name,
      description: description,
    })

  }
  const removeTodo = () => {
    dispath(removeTodoByIndex({
      name: name,
      description: description,
    }))
  }



  const parts = description.split(/#[\wа-яё]+/gi);
  const matches = description.match(/#[\wа-яё]+/gi);

  return (
    <div className='todoCard'>
      <div className='infoCard'>
        <h4>{name}</h4>
        <div className='descriptionCard'>
          {parts.map((el, index) => (
            <React.Fragment key={`el-${index}`}>
              <span>{el}</span>
              {matches && matches[index] && (
                <span className="hashtag">
                  {matches[index]}
                </span>
              )}
            </React.Fragment>
          ))}
        </div>

      </div>
      <div className='svgIcons'>
        <svg className='editsvg' onClick={() => editTodo()} height="91px" viewBox="0 0 91 91" width="91px"><g><path d="M36.682,45.397l-0.002,0.001c-0.002,0.002-0.002,0.004-0.002,0.004c-0.137,0.139-0.24,0.307-0.324,0.486   c-0.023,0.045-0.039,0.086-0.057,0.133c-0.018,0.046-0.043,0.088-0.057,0.137l-4.008,14.826c-0.158,0.588,0.01,1.215,0.439,1.646   c0.324,0.323,0.758,0.498,1.201,0.498c0.148,0,0.299-0.019,0.443-0.06l14.256-3.853c0.094,0.016,0.184,0.055,0.279,0.055   c0.436,0,0.871-0.166,1.203-0.498l23.086-23.088c1.23-1.231,1.91-2.868,1.91-4.609c0-1.741-0.68-3.378-1.91-4.609l-4.154-4.154   c-2.461-2.463-6.76-2.459-9.217,0L36.682,45.397L36.682,45.397z M62.174,24.715c1.174-1.177,3.23-1.176,4.408,0.001l4.154,4.153   c0.59,0.589,0.914,1.372,0.914,2.206s-0.324,1.616-0.914,2.206l-1.25,1.25l-8.564-8.563L62.174,24.715z M58.518,28.372l8.564,8.563   l-18.23,18.231l-8.566-8.564L58.518,28.372z M39.285,50.407l6.137,6.138l-9.135,2.471l2.469-9.136L39.285,50.407z" /><rect height="15.087" transform="matrix(0.7072 0.707 -0.707 0.7072 45.0547 -26.2009)" width="3.4" x="52.459" y="33.749" /><path d="M20.961,74.166h45.74c3.596,0,6.518-2.925,6.518-6.52V44.628H69.82v23.019c0,1.721-1.398,3.119-3.119,3.119H22.662V25.662   h29.012v-3.4H20.961c-0.939,0-1.699,0.761-1.699,1.7v48.503C19.262,73.404,20.021,74.166,20.961,74.166z" /></g></svg>
        <svg className='delsvg' onClick={() => removeTodo()} viewBox="0 0 40 40"><g><path d="M28,40H11.8c-3.3,0-5.9-2.7-5.9-5.9V16c0-0.6,0.4-1,1-1s1,0.4,1,1v18.1c0,2.2,1.8,3.9,3.9,3.9H28c2.2,0,3.9-1.8,3.9-3.9V16   c0-0.6,0.4-1,1-1s1,0.4,1,1v18.1C33.9,37.3,31.2,40,28,40z" /></g><g><path d="M33.3,4.9h-7.6C25.2,2.1,22.8,0,19.9,0s-5.3,2.1-5.8,4.9H6.5c-2.3,0-4.1,1.8-4.1,4.1S4.2,13,6.5,13h26.9   c2.3,0,4.1-1.8,4.1-4.1S35.6,4.9,33.3,4.9z M19.9,2c1.8,0,3.3,1.2,3.7,2.9h-7.5C16.6,3.2,18.1,2,19.9,2z M33.3,11H6.5   c-1.1,0-2.1-0.9-2.1-2.1c0-1.1,0.9-2.1,2.1-2.1h26.9c1.1,0,2.1,0.9,2.1,2.1C35.4,10.1,34.5,11,33.3,11z" /></g><g><path d="M12.9,35.1c-0.6,0-1-0.4-1-1V17.4c0-0.6,0.4-1,1-1s1,0.4,1,1v16.7C13.9,34.6,13.4,35.1,12.9,35.1z" /></g><g><path d="M26.9,35.1c-0.6,0-1-0.4-1-1V17.4c0-0.6,0.4-1,1-1s1,0.4,1,1v16.7C27.9,34.6,27.4,35.1,26.9,35.1z" /></g><g><path d="M19.9,35.1c-0.6,0-1-0.4-1-1V17.4c0-0.6,0.4-1,1-1s1,0.4,1,1v16.7C20.9,34.6,20.4,35.1,19.9,35.1z" /></g></svg>
      </div>
    </div>
  )
}

export default TodoCard